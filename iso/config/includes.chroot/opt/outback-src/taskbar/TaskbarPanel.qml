...
Window {
    ...
    RowLayout {
        ...
        Repeater {
            // Pinned taskbar apps come from the same registry (Apps.qml)
            // the start menu's all-apps list uses, filtered through
            // PinnedApps' user-editable pin/unpin set rather than reading
            // Apps.qml's own "pinned" flags directly, so pinning/unpinning
            // from the start menu or by right-clicking a pinned button here
            // persists instead of just resetting to the shipped defaults.
            model: PinnedApps.pinnedIds

            delegate: PinnedButton {
                required property string modelData
                readonly property var appData: Apps.byId(modelData)
